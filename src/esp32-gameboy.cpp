...
Adafruit_ST7735s st_tft = Adafruit_ST7735s(&SPI, TFT_CS, TFT_DC, TFT_RST);
...
void setup() {
  Serial.begin(115200);
  Serial.println("Starting ESP32 Gameboy Emulator");
  ...
  if (setCpuFrequencyMhz(240)) {
    ...
    st_tft.setRotation(3);
    st_tft.ST7735sPatch();
    tft = new BufferedDisplay(st_tft, TFT_startWrite, TFT_setAddressWindow, TFT_writePixels, TFT_endWrite);
